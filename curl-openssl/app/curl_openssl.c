...
static CURLcode sslctx_function(CURL* curl, void* sslctx, void* parm) {
    ...
    fp = fopen("/usr/local/packages/curl_openssl/cacert.pem", "r");
    if (fp == NULL) {
...
int main(void) {
    CURL* curl;
    CURLcode rv;
    struct File fetch_file = {// Path to the file which will store the fetched web content
    ...
    if (curl) {
        ...
        if (rv == CURLE_OK)
            syslog(LOG_INFO,
                   "*** 1. Transfer Passed: Unexpected result, transfer without certificate should "
                   "not pass ***");
        else
            syslog(LOG_INFO,
                   "*** 1. Transfer Failed: Expected result, transfer without certificate should "
                   "fail ***");
        ...
        if (rv == CURLE_OK)
            syslog(LOG_INFO,
                   "*** 2. Transfer Succeeded: Expected result, transfer with CA-cert should pass "
                   "***");
        else
