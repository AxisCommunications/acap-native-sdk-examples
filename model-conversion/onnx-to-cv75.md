...
# onnx model -> cv75 native model

This example shows how to convert an .onnx model to a native cv75 model for execution on cv75 devices.
...
2. Inside the CNNGen Toolchain container, modify and run the following commands to generate images for quantization and to convert the model to the cv75 native format:

    ```bash
    #!/bin/bash
    INPUT_MODEL=my_model.onnx
    OUTPUT_MODEL=my_model_cv75.bin
    IMAGE_SIZE=(224 224)
